...
function getOrCreateChatSessionId() {
  const key = 'dobby_chat_session_id';
  // Start a fresh chat session on every page load to avoid carrying stale backend state.
...
async function sendChat() {
  ...
  try {
    const response = await fetch('/chat', {
      ...
      body: JSON.stringify({ messages: conversationMessages, session_id: chatSessionId })
    });
    ...
    let botText = "Please share more details about the shirt design so I can complete the parameters.";
    ...
    // Prefer server-guided next question when required inputs are missing.
    // This enforces one-by-one slot collection.
    if (data.next_question) {
      botText = data.next_question;
    } else if (data.reply) {
      ...
      if (!isJson) {
        ...
        botText = "Design properties are updated below. Tell me what you want to refine next.";
      }
    }
